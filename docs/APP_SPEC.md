...
> **Partially implemented.** This is the quality bar QuoteForge proposals must hit. Done so far: the scope-prose tone/terminology/trust block in the LLM system prompt (24.1/24.2/24.4), clean multi-paragraph PDF scope (24.5), and the optional add-on split end-to-end (24.4). Still **new work**: a hard terminology lint at finalize, the real-world risk-assumption audit rules (24.3), assembly confidence scoring (24.6), and the engineering sanity checks (24.7) — several of which depend on electrician-reviewed data. Build it in the §3 spirit: the model improves *language and presentation*, never the numbers, and never invents code/technical facts.
