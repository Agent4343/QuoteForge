...
Pure Python. Rule families: missing items (permit-when-service-change, AFCI for new dwelling circuits, travel for long jobs, HQ coordination for QC service changes), margin (below minimum, labour-ratio band), per-assembly labour plausibility band, stale pricing (>90 days), scope-risk language, provincial-specific (QC customer-language, QC transition warning, ON ESA notification), **real-world estimating protection (§24.3)**: utility-locate-required, excavation-conditions-assumed, concealed-access-assumed, material-price-volatility, service-capacity-verify, customer-supplied-equipment, permit/inspection-timeline, weather-delay, and **terminology enforcement (§24.2)**: `TERMINOLOGY_SERVICE_MISUSE` (utility-"service" wording with no service-upgrade work). Each rule returns `None`/`[]` or a `Flag`. Critical flags set `pdf_blocked`; the §24.2/§24.3 rules are info/warn only (never block) and consider the committed (non-optional) scope. The terminology lint also re-runs at finalize (`quote_service.ensure_terminology_flag`) to catch scope written after the last audit. Override is logged.
...
The system must distinguish, and use correctly: utility service · feeder · subfeed · distribution panel · disconnect · subpanel · branch circuits. A detached structure fed from the house is **never** a "new utility service" — prefer "garage feeder" / "garage electrical feed" / "garage distribution panel."
**Implemented** at two layers: (1) the §24.1 system-prompt block instructs the model to use correct terms and self-correct; (2) a deterministic lint — `TERMINOLOGY_SERVICE_MISUSE`, a warn flag that fires when the customer scope uses utility-"service" wording but the quote has no service-upgrade (SERVICE-category) assembly. It runs as a normal audit rule (visible while editing) and is re-checked at finalize via `quote_service.ensure_terminology_flag` so a scope the LLM wrote after the last audit is still linted before the PDF. It's a warn (never blocks) to avoid false-positive gating on free text. A broader lexicon (feeder/subfeed/disconnect mix-ups) is a future extension.
...
**Scaffolded:** the assembly schema gains `review_count`, `provinces_reviewed`, `last_field_validation`, and `customer_risk` (low|medium|high), all with safe defaults so nothing is claimed without review. `assemblies/confidence.py` **derives** a `Confidence` level (unreviewed/low/moderate/high) deterministically from that metadata — never hand-set — so it can't drift from reality (every assembly is `unreviewed` today). Surfaced internally on `/api/assemblies` and on the dashboard assembly table (a Confidence badge + a "needs review" ⚠ that combines low confidence, high edit-rate, or high customer-risk). **Pending review:** the metadata itself is populated when an electrician signs off; `actual-vs-estimated variance tracking` is not yet built (needs real job-cost feedback).
